const ChatApp = {
  ...
  getBotResponse(userMessage, profile) {
    ...
    // --- 1. DETECCIÓN DE PREGUNTAS DE COMIDA (CORREGIDO PARA GENERALES) ---
    if (
      msg.includes('comer') || msg.includes('cocinar') || msg.includes('receta') ||
      msg.includes('cena') || msg.includes('cenar') || 
      msg.includes('almuerzo') || msg.includes('almorzar') || 
      msg.includes('desayuno') || msg.includes('desayunar') || 
      msg.includes('merienda') || msg.includes('merendar')
    ) {
      
      let categoriaBuscada = '';
      let momentoDelDia = '';

      // Primero revisamos si especificó algún momento
      if (msg.includes('desayno') || msg.includes('desayunar') || msg.includes('desayuno')) {
        ...
        // SI ES UNA PREGUNTA GENERAL ("QUÉ PUEDO COMER"), EVALÚA LA HORA REAL:
        const hora = new Date().getHours();
        if (hora >= 6 && hora < 12) { 
          categoriaBuscada = 'desayuno'; 
          momentoDelDia = 'un desayuno para arrancar el día'; 
        } else if (hora >= 12 && hora < 16) { 
          categoriaBuscada = 'almuerzo'; 
          momentoDelDia = 'un almuerzo para recargar energías'; 
        } else if (hora >= 16 && hora < 20) { 
          categoriaBuscada = 'meriendas'; 
          momentoDelDia = 'una merienda para la tarde'; 
        } else { 
          categoriaBuscada = 'cena'; 
          momentoDelDia = 'una cena liviana antes de dormir'; 
        }
      }
      ...
      if (opcionesAptas.length > 0) {
        ...
        const introsRecetas = [
          `¡Perfecto! Pensando en tu perfil, para este momento te sugiero **{momento}**.`,
          `Mirá, basándome en tus metas y restricciones de hoy, acá tenés opciones para **{momento}**:`,
          `Chequeando tu menú personalizado para este horario, te dejo ideas para **{momento}**:`
        ];
        ...
        respuesta += `<br><br>¿Te copa alguna de estas opciones o preferís otra categoría?`;
        return respuesta;
      } else {
        return `Estuve buscando ideas de tipo ${categoriaBuscada} para sugerirte, pero tus filtros de alergias o disgustos bloquean mis opciones actuales. ¿Querés probar buscando otra cosa?`;
      }
    }
    ...
    if (msg.includes('no me gusta') || msg.includes('alerg') || msg.includes('evitar') || msg.includes('asco') || msg.includes('odio')) {
      ...
      if (todasLasRestricciones.length > 0) {
        return `Ya tengo agendado que preferís evitar: <b>${todasLasRestricciones.join(', ')}</b>. Ninguna propuesta va a incluir eso, tranqui.`;
      }
      return "Si hay algún alimento que te caiga mal, podés cargarlo reiniciando tu perfil.";
    }
